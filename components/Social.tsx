  ...
  return (
    <section id="contact" className="overflow-hidden py-2 md:py-4 px-3 md:px-6 text-center bg-[var(--bg3)]">
      <div dir="ltr" className="max-w-[1000px] mx-auto flex justify-center md:gap-6">
        {items.map(({ Icon, label, href }) => (
          <a key={label} href={href} target={href.startsWith('http') ? '_blank' : undefined}
             className="flex flex-col items-center gap-1 md:gap-2 max-md:px-1 md:px-4 py-2 text-[var(--champagne)] hover:text-[var(--champagne-deep)] transition-colors">
        ...
        <div className="relative">
          <button onClick={() => setSites(!sites)} className="flex flex-col items-center gap-1 md:gap-2 max-md:px-1 md:px-4 py-2 text-[var(--champagne)] hover:text-[var(--champagne-deep)] transition-colors">
          ...
          {sites && (
            <div dir="rtl" className="absolute bottom-full right-1/2 translate-x-1/2 mb-2 flex flex-col gap-2 rounded-[6px] bg-[var(--card)] border border-[var(--divider)] p-4 text-sm whitespace-nowrap z-10">
              <a href="https://howkworld.com/ar" target="_blank" className="hover:text-[var(--champagne)]">HOWK</a>
              <a href="https://edmarksa.com/" target="_blank" className="hover:text-[var(--champagne)]">EDMARK</a>
            </div>
          )}
        </div>
      </div>
      <p dir="ltr" className="mt-1 md:mt-3 text-[12px] md:text-sm text-[var(--muted)] tracking-wider flex items-center justify-center flex-wrap gap-2 md:gap-3">
        <a href="mailto:EDMARK@live.in" className="flex items-center gap-2 hover:text-[var(--champagne)] transition-colors">
          <FaEnvelope className="size-3.5 md:size-4 text-[var(--champagne)]" />
          EDMARK@live.in
        </a>
        <span className="mx-1">•</span>
        <a href="https://omar-alomar.com" className="flex items-center gap-2 hover:text-[var(--champagne)] transition-colors">
          <FaGlobe className="size-3.5 md:size-4 text-[var(--champagne)]" />
          omar-alomar.com
        </a>
      </p>
    </section>
