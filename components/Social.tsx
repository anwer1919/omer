...
export default function Social() {
  ...
  return (
    <section id="contact" className="overflow-hidden py-1 md:py-2 px-3 md:px-6 text-center bg-[var(--bg3)]">
      <div dir="ltr" className="max-w-[1000px] mx-auto flex justify-center md:gap-6">
        {items.map(({ Icon, label, href }) => (
          <a key={label} href={href} target={href.startsWith('http') ? '_blank' : undefined}
             className="flex flex-col items-center gap-1 md:gap-1.5 max-md:px-1 md:px-4 py-1.5 text-[#d9a066] hover:text-[#e8b880] transition-colors">
            <Icon className="size-4 md:size-5" />
            <span dir="ltr" className="text-[7px] md:text-[11px] max-md:tracking-normal md:tracking-wider text-[#a89080]">{label}</span>
          </a>
        ))}
        <div className="relative">
          <button onClick={() => setSites(!sites)} className="flex flex-col items-center gap-1 md:gap-1.5 max-md:px-1 md:px-4 py-1.5 text-[#d9a066] hover:text-[#e8b880] transition-colors">
          ...
          {sites && (
            <div dir="rtl" className="absolute bottom-full right-1/2 translate-x-1/2 mb-2 flex flex-col gap-2 rounded-[6px] bg-[var(--card)] border border-[#3a2a1a] p-3 text-sm whitespace-nowrap z-10">
      ...
      <p dir="ltr" className="mt-0.5 md:mt-2 text-[12px] md:text-sm text-[#a89080] tracking-wider flex items-center justify-center flex-wrap gap-2 md:gap-3">
